...
/**
 * News entry metadata (mirrors `NewsPostMeta` from lib/news.ts but without the
 * Node.js dependencies, so it is safe in a client component).
 */
...
const typeBadgeStyles: Record<NewsCardMeta['type'], string> = {
    ...
    <Link href={`/news/${post.slug}`} className="group block h-full">
      <article className="h-full overflow-hidden rounded-xl bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
        <div className="relative aspect-[16/9] overflow-hidden">
          ...
          />
          <span
            className={`absolute top-3 left-3 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider rounded-full backdrop-blur-sm ${typeBadgeStyles[post.type]}`}
          >
            {typeLabel}
          </span>
        </div>
