...
- Weekly newsletters belong in this repository under `newsletters/YYYY-MM-DD/`. Read `docs/newsletters.md` and `scripts/newsletters/README.md`. The fortnightly ecosystem series remains governed by dl-social’s `docs/editorial/nostr-ecosystem-watch.md`.
...
- The user authorized weekly newsletter delivery to all active subscribers on 11 September 2026. Prepare all seven editions, validate, deploy through GitHub Actions, dry-run the deployed sender, then dispatch newsletter.yml with send=true. Match stored locales, honor opt-outs and reconcile uncertain sends rather than retrying. Public blog/social publishing still requires its separate authority. Access mail credentials only inside the server-side sender, never for research.
