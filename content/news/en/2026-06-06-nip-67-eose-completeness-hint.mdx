...
Compatibility runs in both directions. A relay that adds this NIP keeps sending a two-element `EOSE` to clients that don't understand the hint, because clients that don't implement NIP-67 index `EOSE` by position and JSON parsers accept the trailing array element without complaint. A client that adds support for the hint gets nothing extra from relays that haven't implemented it, and simply keeps using its existing pagination logic.
